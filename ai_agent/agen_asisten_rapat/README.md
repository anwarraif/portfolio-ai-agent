...
cd 2.ai_agent_basic/agen_asisten_rapat
```
...
note: 
Serp API (Search Engine Results Page API) is a service that allows applications to programmatically perform Google searches and get search results in JSON format.
❌ Without Serp: GPT-4 only knows info up to the training limit
✅ With Serp: Real-time info from Google search results