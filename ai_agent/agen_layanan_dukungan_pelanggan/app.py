...
# running syntax: streamlit run customer_support_agent.py
# Set up the Streamlit App

st.set_page_config(
    page_title="Agen Layanan Pelanggan dengan Memori 🛒",
    page_icon="../../favicon.ico",
    layout="wide"
)
st.caption("Chat dengan asisten layanan pelanggan yang mengingat interaksi-interaksi anda sebelumnya.")

# Set the OpenAI API key
openai_api_key = st.text_input("Masukan API OpenAI", type="password")
...
if openai_api_key:
    ...
    class CustomerSupportAIAgent:
        def __init__(self):
            config = {
                ...
                "provider": "qdrant",
                "config": {
                    "host": "localhost",               # Host Qdrant
                    "port": 6333,                      # Port Qdrant
                    "collection_name": "customer_support",  # Nama koleksi untuk menyimpan data
                    "embedding_model_dims": 1536,      # Dimensi embedding (sesuaikan dengan model embedding Anda)
                }
            },
            }
    ...
    st.sidebar.title("Masukan ID Pelanggan:")
    previous_customer_id = st.session_state.get("previous_customer_id", None)
    customer_id = st.sidebar.text_input("Masukan ID Customer")
    ...
    if st.sidebar.button("Generate Synthetic Data"):
        if customer_id:
            with st.spinner("Generate data pelanggan..."):
                st.session_state.customer_data = support_agent.generate_synthetic_data(customer_id)
            st.sidebar.success("Synthetic data generated berhasil!")
        else:
            st.sidebar.error("Masukan ID Pelanggan terlebih dahulu.")

    if st.sidebar.button("View Customer Profile"):
        if st.session_state.customer_data:
            st.sidebar.json(st.session_state.customer_data)
        else:
            st.sidebar.info("Belum ada data pelanggan. Tekan 'Generate Synthetic Data' terlebih dahulu.")
    ...
    if st.sidebar.button("View Memory Info"):
        if customer_id:
            ...
            if memories:
                ...
                st.sidebar.info("Tidak ada memori yang ditemukan untuk ID pelanggan ini.")
        else:
            st.sidebar.error("Silahkan masukan ID Pelanggan untuk melihat info memori.")
    ...
    if query and customer_id:
        ...
        st.error("Masukan ID Pelanggan untuk memulai obrolan.")

else:
    st.warning("Masukan API openAI.")