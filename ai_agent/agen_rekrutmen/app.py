...
def schedule_interview(scheduler: Agent, candidate_email: str, email_agent: Agent, role: str) -> None:
    ...
    Send a clean, user-friendly confirmation email.
    """
    try:
        # --- Waktu Interview ---
        ...
        meeting_response = scheduler.run(
            ...
            - Duration: 60 minutes
            The meeting must be between 9 AM - 5 PM Jakarta time.
            """
        )
        ...
        try:
            ...
            if raw_response.startswith("{"):  # kemungkinan JSON
            ...
            else:  # fallback: cari URL di string
        ...
        email_agent.run(
            f"""
            Send an interview confirmation email to {candidate_email} with these details:

            Subject: Interview Confirmation – {role} Position

            Dear Candidate,

            We are pleased to invite you to the technical interview for the {role} role. 
            ...
            Important Notes:  
            - Please join the meeting 5 minutes early to ensure no technical issues.  
            - If you need timezone conversions, use: https://www.timeanddate.com/worldclock/converter.html  
            - Be confident, prepare well, and we wish you the best of luck!  
        ...
        st.success("✅ Interview scheduled successfully! Confirmation email sent.")
